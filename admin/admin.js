(function () {
    ...
    function buildLgaRows() {
        ...
        lgaRows = lgas.map(function (lga) {
            ...
            let verifiedCoveredWards = 0;
            let verifiedCoveredPollingUnits = 0;

            wards.forEach(function (ward) {
                const units = Array.isArray(ward.pollingUnits) ? ward.pollingUnits : [];
                ...
                units.forEach(function (unit) {
                    const key =
                        ...
                        "||" +
                        normalize(unit.name);
                    const unitMembers = memberIndexes.byPollingUnit.get(key) || [];
                    const approvedMembers = unitMembers.filter(function (m) {
                        return normalize(m.membership_status) === "APPROVED";
                    });

                    if (approvedMembers.length > 0) {
                        verifiedCoveredPollingUnits += 1;
                        wardVerified = true;
                    }
                });

                if (wardVerified) verifiedCoveredWards += 1;
            });

            const verifiedCoverage = percent(
                verifiedCoveredPollingUnits,
                totalPollingUnits
            );
            ...
            return {
                ...
                verifiedCoveredWards: verifiedCoveredWards,
                verifiedCoveredPollingUnits: verifiedCoveredPollingUnits,
                verifiedCoverage: verifiedCoverage,
                verifiedStatus: statusForCoverage(
                    verifiedCoveredPollingUnits,
                    totalPollingUnits
                )
            };
    ...
    /* =========================================================
       PHASE 2D: VERIFIED / APPROVED COVERAGE INTELLIGENCE
    ========================================================= */

    function verifiedStatusForUnit(members) {
    ...
    function verifiedStatusBadge(status) {
        const labels = {
            verified: "VERIFIED",
    ...
    function getUnitMembers(lga, ward, unit) {
        const key =
            normalize(lga.name) +
            "||" +
            normalize(ward.name) +
            "||" +
            normalize(unit.name);
        return memberIndexes.byPollingUnit.get(key) || [];
    }

    function getVerifiedCoverageData() {
        let totalPollingUnits = 0;
        let verifiedPollingUnits = 0;
        let registeredPollingUnits = 0;
        let pendingOnlyPollingUnits = 0;
        let rejectedOnlyPollingUnits = 0;
        let verifiedWards = 0;
        let registeredWards = 0;

        const lgas = lgaRows.map(function (lga) {
            let registeredPUs = 0;
            let verifiedPUs = 0;
            let pendingOnlyPUs = 0;
            let rejectedOnlyPUs = 0;
            let registeredWardCount = 0;
            let verifiedWardCount = 0;

            lga.wards.forEach(function (ward) {
                const units = Array.isArray(ward.pollingUnits) ? ward.pollingUnits : [];
                let wardRegistered = false;
                let wardVerified = false;

                units.forEach(function (unit) {
                    totalPollingUnits += 1;
                    const members = getUnitMembers(lga, ward, unit);
                    const status = verifiedStatusForUnit(members);

                    if (members.length > 0) {
                        registeredPollingUnits += 1;
                        registeredPUs += 1;
                        wardRegistered = true;
                    }
                    if (status === "verified") {
                        verifiedPollingUnits += 1;
                        verifiedPUs += 1;
                        wardVerified = true;
                    } else if (status === "pending") {
                        pendingOnlyPollingUnits += 1;
                        pendingOnlyPUs += 1;
                    } else if (status === "rejected") {
                        rejectedOnlyPollingUnits += 1;
                        rejectedOnlyPUs += 1;
                    }
                });
            ...
            return {
                name: lga.name,
                totalWards: lga.totalWards,
                registeredWardCount: registeredWardCount,
                verifiedWardCount: verifiedWardCount,
                totalPollingUnits: lga.totalPollingUnits,
                registeredPUs: registeredPUs,
                verifiedPUs: verifiedPUs,
                pendingOnlyPUs: pendingOnlyPUs,
                rejectedOnlyPUs: rejectedOnlyPUs,
                registeredCoverage: percent(registeredPUs, lga.totalPollingUnits),
                verifiedCoverage: percent(verifiedPUs, lga.totalPollingUnits),
                verifiedStatus: statusForCoverage(verifiedPUs, lga.totalPollingUnits)
            };
        });
        ...
        return {
            lgas: lgas,
            totalPollingUnits: totalPollingUnits,
            registeredPollingUnits: registeredPollingUnits,
            verifiedPollingUnits: verifiedPollingUnits,
            pendingOnlyPollingUnits: pendingOnlyPollingUnits,
            rejectedOnlyPollingUnits: rejectedOnlyPollingUnits,
            registeredWards: registeredWards,
            verifiedWards: verifiedWards
        };
    }

    function renderVerifiedCoverage() {
        const data = getVerifiedCoverageData();

        $("verifiedPollingUnits").textContent = data.verifiedPollingUnits.toLocaleString();
        $("verifiedWards").textContent = data.verifiedWards.toLocaleString();
        $("verifiedLgas").textContent = data.lgas.filter(function (lga) {
            return lga.verifiedPUs > 0;
        }).length.toLocaleString();
        $("verifiedMembers").textContent = allMembers.filter(function (m) {
            return normalize(m.membership_status) === "APPROVED";
        }).length.toLocaleString();
        $("verifiedCoveragePercent").textContent =
            percent(data.verifiedPollingUnits, data.totalPollingUnits) + "%";
        $("verifiedCoverageSub").textContent =
            data.verifiedPollingUnits.toLocaleString() +
            " of " + data.totalPollingUnits.toLocaleString() +
            " polling units have an approved member";

        $("pendingOnlyUnits").textContent = data.pendingOnlyPollingUnits.toLocaleString();
        $("rejectedOnlyUnits").textContent = data.rejectedOnlyPollingUnits.toLocaleString();

        const rows = data.lgas.slice().sort(function (a, b) {
            if (b.verifiedPUs !== a.verifiedPUs) return b.verifiedPUs - a.verifiedPUs;
            if (b.totalPollingUnits - b.verifiedPUs !== a.totalPollingUnits - a.verifiedPUs) {
                return (b.totalPollingUnits - b.verifiedPUs) - (a.totalPollingUnits - a.verifiedPUs);
            }
            return a.name.localeCompare(b.name);
        });

        $("verifiedCoverageBody").innerHTML = rows.map(function (row) {
            const status = row.verifiedPUs > 0 ? "verified" :
                (row.pendingOnlyPUs > 0 ? "pending" :
                (row.rejectedOnlyPUs > 0 ? "rejected" : "none"));

            return `
                <tr>
                    <td><span class="coverage-name">${escapeHtml(row.name)}</span></td>
                    <td>${row.registeredWardCount}/${row.totalWards}</td>
                    <td>${row.verifiedWardCount}</td>
                    <td>${row.totalPollingUnits.toLocaleString()}</td>
                    <td>${row.registeredPUs}</td>
                    <td><strong>${row.verifiedPUs}</strong></td>
                    <td>${(row.totalPollingUnits - row.verifiedPUs).toLocaleString()}</td>
                    <td>
                        <span class="coverage-progress">
                            <span style="width:${Math.min(row.verifiedCoverage, 100)}%"></span>
                        </span>
                        <span class="coverage-percent">${row.verifiedCoverage}%</span>
                    </td>
                    <td>${verifiedStatusBadge(status)}</td>
                </tr>
            `;
        }).join("");
    }
