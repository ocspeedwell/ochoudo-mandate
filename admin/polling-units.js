...
console.log("OMG Command Centre PHASE-3D-FINAL-FIX1 loaded");
    ...
    function getPuProfile(puKey) {
        ...
        const activityRecords = activities.filter(a => normalize(a.lga_name) === row.lgaKey && normalize(a.ward_name) === normalize(row.wardName) && normalize(a.polling_unit_name) === normalize(row.puName) && isRecentActivity(a)).sort((a,b)=>compareText(b.activity_date, String(a.activity_date)));
    ...
    function renderPuReadiness(){
        ...
        rows.sort((a,b)=>{ if(sort==="score-asc") return a.score-b.score || compareText(a.puName, b.puName); if(sort==="gap-desc") return b.gap-a.gap || compareText(a.puName, b.puName); if(sort==="pu-asc") return compareText(a.puName, b.puName); return b.score-a.score || compareText(a.puName, b.puName); });
