...
console.log("OMG Command Centre PHASE-3D-FINAL-FIX1 loaded");
    ...
    function renderWardReadiness(){
        ...
        rows.sort(function(a,b){
            if(sort==="score-asc") return a.score-b.score || String(a.lgaName || "").localeCompare(b.lgaName) || String(a.wardName || "").localeCompare(b.wardName);
            if(sort==="gap-desc") return b.gap-a.gap || String(a.lgaName || "").localeCompare(b.lgaName);
            if(sort==="ward-asc") return String(a.wardName || "").localeCompare(b.wardName) || String(a.lgaName || "").localeCompare(b.lgaName);
            return b.score-a.score || String(a.lgaName || "").localeCompare(b.lgaName) || String(a.wardName || "").localeCompare(b.wardName);
        });
    ...
    function renderReadiness() {
        ...
        rows.sort(function(a,b) {
            if (sort === "score-asc") return a.score - b.score || String(a.name || "").localeCompare(b.name);
            if (sort === "gap-desc") return b.gap - a.gap || String(a.name || "").localeCompare(b.name);
            if (sort === "lga-asc") return String(a.name || "").localeCompare(b.name);
            return b.score - a.score || String(a.name || "").localeCompare(b.name);
        });
    ...
    function renderHealth() {
        ...
        filtered.sort(function(a,b){
            if(sort === "health-asc") return a.health-b.health || String(a.name || "").localeCompare(b.name);
            if(sort === "activity-desc") return String(b.latestActivity).localeCompare(String(a.latestActivity)) || b.health-a.health;
            if(sort === "gap-desc") return b.gap-a.gap || String(a.name || "").localeCompare(b.name);
            return b.health-a.health || String(a.name || "").localeCompare(b.name);
        });
    ...
    function renderPuReadiness(){
        ...
        rows.sort((a,b)=>{ if(sort==="score-asc") return a.score-b.score || String(a.puName || "").localeCompare(b.puName); if(sort==="gap-desc") return b.gap-a.gap || String(a.puName || "").localeCompare(b.puName); if(sort==="pu-asc") return String(a.puName || "").localeCompare(b.puName); return b.score-a.score || String(a.puName || "").localeCompare(b.puName); });
